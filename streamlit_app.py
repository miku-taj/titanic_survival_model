...
with st.form("user_input_form"):
    ...
    sibsp_input = st.number_input("Братья/сестры или супруг(а) на борту (SibSp)", value=0, min_value=float(data['SibSp'].min()), max_value=float(data['SibSp'].max()), step=1.0)
    parch_input = st.number_input("Родители/дети на борту (Parch)", value=0, min_value=int(data['Parch'].min()), max_value=int(data['Parch'].max()), step=1)
